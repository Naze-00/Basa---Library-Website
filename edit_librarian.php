...
if (isset($_POST['submit'])) {
    ...
    $sql = "UPDATE `librarian` SET `FirstName`=?, `LastName`=?, `Email`=?, `Pass`=? WHERE LibrarianId =?";
    $stmt = mysqli_prepare($conn, $sql);
    mysqli_stmt_bind_param($stmt, "ssssi", $first_name, $last_name, $email, $password, $id);
    mysqli_stmt_execute($stmt);

    if (mysqli_stmt_affected_rows($stmt) > 0) {
        header("Location:./librarian.php?msg=Record has been changed successfully!");
    } else {
...
$id = $_GET['id'];
$sql = "SELECT * FROM `librarian` WHERE LibrarianId = $id LIMIT 1";
        ...
        <div class="container d-flex justify-content-center">
            <form action="" method="post" style="width: 50w; min-width: 300px;">
                ...
                <div class="row">
                    ...
                    <div class="col">
                        <a href="librarian.php" class="btn btn-block mb-4 w-100 text-white btn-dark">Cancel</a>
                    </div>
                </div>
            </form>
        </div>
